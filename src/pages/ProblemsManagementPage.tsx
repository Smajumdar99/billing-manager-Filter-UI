...
import { ArrowLeftIcon, MagnifyingGlassIcon, PlusCircleIcon, DocumentTextIcon, PencilIcon, TrashIcon, PrinterIcon } from '@heroicons/react/24/outline';
import { Button } from '@/components/atoms/Button';
import { Badge } from '@/components/atoms/Badge';
import { Tabs } from '@/components/atoms/Tabs';
import { Dialog, DialogContent, DialogTitle, DialogHeader } from '@/components/atoms/Dialog/dialog';
import {
  Select,
  SelectContent,
  SelectItem,
      ...
      >
        ...
        >
          ...
          <DialogTitle className="sr-only">
            {linkEncountersDialog.showAddDiagnosis ? 'Add New Diagnosis' : 'Link Encounters to Problem'}
          </DialogTitle>
          {/* ... */}
          
          {linkEncountersDialog.showAddDiagnosis ? (
            // Add Condition Mode - Show AddConditionDialog content
            <div className="bg-white rounded-xl border border-gray-200 shadow-lg m-6">
              {/* Compact Header with Back Arrow and Title */}
          ...
          {/* Main two-section layout - Only show when not in Add Diagnosis mode */}
          {!linkEncountersDialog.showAddDiagnosis && (
            <>
            <div className="flex flex-1 min-h-0 overflow-hidden gap-6 p-4">
            {/* Left Section - Dynamic based on mode */}
            <div className="w-[400px] min-w-[400px] bg-white rounded-xl border border-gray-200 shadow-sm h-full flex flex-col">
              {linkingMode === 'clinical-to-encounters' ? (
                // Mode 1: Clinical Items on Left
                <>
                  <div className="p-4 border-b border-gray-100">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="text-sm font-semibold text-gray-900">Clinical Items</h3>
                        <p className="text-xs text-gray-500 mt-1">Select items to link encounters</p>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
                ...
                // Mode 2: Encounters on Left
                <>
                  <div className="p-4 border-b border-gray-100">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="text-sm font-semibold text-gray-900">Encounters</h3>
                        <p className="text-xs text-gray-500 mt-1">Select encounters to link clinical items</p>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
              ...
              {linkingMode === 'clinical-to-encounters' ? (
                // Mode 1: Clinical Items Content
                <>
                  {/* Tabs for different clinical categories */}
                      ...
                      {problems.map((problem) => (
                        <div
                          ...
                          className={`p-3 border rounded-lg cursor-pointer transition-colors ${
                            problem.id === linkEncountersDialog.problemId
                              ? 'border-blue-500 bg-blue-50'
                              : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                          }`}
                          onClick={() => setLinkEncountersDialog(prev => ({ ...prev, problemId: problem.id, problemTitle: problem.title }))}
                        >
                ...
                // Mode 2: Encounters Content - Show linked encounters for selected problem
                linkEncountersDialog.problemId ? (
                  <>
                    <div className="p-4 border-b border-gray-100">
                      <h4 className="text-sm font-semibold text-gray-900 mb-2">
                        Encounters Linked to "{linkEncountersDialog.problemTitle}"
                      </h4>
                      <p className="text-xs text-gray-500">Select an encounter to manage its clinical items</p>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4">
                      <div className="space-y-2">
                        {(() => {
                          const selectedProblem = problems.find(p => p.id === linkEncountersDialog.problemId);
                          const linkedEncounters = selectedProblem?.linkedEncounters || [];
                          
                          if (linkedEncounters.length === 0) {
                            return (
                              <div className="text-center text-gray-400 mt-20">
                                <div className="text-sm">No encounters linked to this problem</div>
                                <div className="text-xs mt-2">Switch to "Clinical Items → Encounters" mode to link encounters</div>
                              </div>
                            );
                          }
                          
                          return linkedEncounters.map((encounter) => (
                            <div
                              key={encounter.id}
                              className={`p-3 border rounded-lg cursor-pointer transition-colors ${
