...
const getPriorityBadgeStyles = (priority: string) => {
  switch (priority) {
    case 'High':
      return "bg-red-50 text-red-700 border-none px-2.5 py-0.5 rounded-full text-xs font-medium";
    case 'Medium':
      return "bg-amber-50 text-amber-700 border-none px-2.5 py-0.5 rounded-full text-xs font-medium";
    case 'Low':
      return "bg-green-50 text-green-700 border-none px-2.5 py-0.5 rounded-full text-xs font-medium";
    default:
      return "bg-gray-50 text-gray-700 border-none px-2.5 py-0.5 rounded-full text-xs font-medium";
  }
};
...
const getStatusBadgeStyles = (status: string) => {
  switch (status) {
    case 'Completed':
      return "bg-green-50 text-green-700 border-none px-2.5 py-0.5 rounded-full text-xs font-medium";
    case 'In Progress':
      return "bg-blue-50 text-blue-700 border-none px-2.5 py-0.5 rounded-full text-xs font-medium";
    case 'Pending':
      return "bg-amber-50 text-amber-700 border-none px-2.5 py-0.5 rounded-full text-xs font-medium";
    case 'Overdue':
      return "bg-red-50 text-red-700 border-none px-2.5 py-0.5 rounded-full text-xs font-medium";
...
const TaskCard: React.FC<{ task: Task; onSelect: (task: Task) => void }> = ({ task, onSelect }) => {
  return (
    <div 
      className="bg-white rounded-lg border border-slate-100 p-3 mb-2 hover:bg-slate-50 active:bg-slate-100 transition-colors"
      onClick={() => onSelect(task)}
    >
      <div className="flex justify-between items-start mb-1.5">
        <h3 className="text-sm font-semibold text-slate-900 flex-1 mr-2">
          {task.title || <span className="text-slate-400 italic font-normal">No subject</span>}
        </h3>
        <span className={cn('shrink-0', getPriorityBadgeStyles(task.priority))}>
      ...
      <div className="grid grid-cols-2 gap-1.5 text-[11px] text-slate-500 mb-2.5">
      ...
      <div className="flex items-center justify-between">
        <span className={cn(getStatusBadgeStyles(task.status))}>
          {task.status}
        </span>
        <div className="flex gap-2">
          <button className="text-blue-600 hover:text-blue-800 text-[11px] font-medium px-1.5 py-0.5 rounded hover:bg-blue-50 transition-colors">
            Reply
          </button>
          <button className="text-slate-400 hover:text-green-600 p-1 rounded hover:bg-green-50 transition-colors">
...
const AllTasksSection: React.FC = () => {
  ...
  const columnDefs = [
    ...
    },
    {
      ...
      cellRenderer: (params: any) => (
        <div className="text-sm font-semibold text-slate-900 truncate" title={params.data.title || 'No subject'}>
          {params.data.title || <span className="text-slate-400 italic font-normal">No subject</span>}
        </div>
      )
    },
    {
      ...
      filter: 'agTextColumnFilter',
      cellRenderer: (params: any) => (
        <div className="text-xs text-slate-500 line-clamp-2" title={params.data.description}>
    ...
    },
    {
      ...
      cellRenderer: (params: any) => (
        <div className="flex gap-1.5 items-center justify-end">
          <button
            className="text-blue-600 hover:text-blue-800 text-[11px] font-medium px-2 py-1 rounded-md hover:bg-blue-50 transition-colors"
            tabIndex={0}
            title="Reply to task"
          >
            Reply
          ...
          <button
            className="text-slate-400 hover:text-green-600 p-1.5 rounded-md hover:bg-green-50 transition-colors"
            title="Complete task"
          >
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden border border-slate-200 mt-2">
      <div 
        className="px-4 py-3.5 border-b border-slate-100 flex justify-between items-center cursor-pointer hover:bg-slate-50/50 transition-colors"
