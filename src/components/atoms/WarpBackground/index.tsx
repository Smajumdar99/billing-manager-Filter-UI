...
const Beam = ({
  ...
  // Generate pastel colors based on our primary blue theme
  const colors = [
    "hsl(196, 100%, 85%)", // Light blue
    "hsl(196, 80%, 90%)",  // Softer blue
    "hsl(206, 70%, 88%)",  // Sky blue
    "hsl(186, 60%, 90%)",  // Cyan tint
    "hsl(216, 50%, 92%)",  // Baby blue
  ];
  
  const randomColor = colors[Math.floor(Math.random() * colors.length)];
  const ar = Math.floor(Math.random() * 2) + 1.5; // More consistent aspect ratios
      ...
      style={
        {
          ...
          "--width": `${width}`,
          "--aspect-ratio": `${ar}`,
          "--background": `linear-gradient(${randomColor}, transparent)`,
        } as React.CSSProperties
      }
      className={cn(
        "absolute left-[var(--x)] top-0",
        "[aspect-ratio:1/var(--aspect-ratio)]",
        "[background:var(--background)]",
        "[width:var(--width)]",
        "opacity-60" // Add some transparency
      )}
